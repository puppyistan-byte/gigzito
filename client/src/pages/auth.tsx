          ...
          <TabsContent value="login">
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <button type="button" className="link-red" data-testid="button-forgot-password"
                  onClick={() => { setForgotOpen(true); setForgotEmail(loginEmail); setForgotSent(false); }}>
                  Forgot password?
                </button>
              </div>
            ...
            {forgotOpen && (
              <div className="mt-4 border border-border rounded-xl p-4 space-y-3 bg-muted/30">
                {forgotSent ? (
                  <div className="text-center space-y-2 py-2">
                    <MailCheck className="w-8 h-8 text-green-500 mx-auto" />
                    <p className="text-sm font-semibold">Check your inbox</p>
                    <p className="text-xs text-muted-foreground">If that email has an account, a reset link is on its way. Check your spam folder too.</p>
                    <button className="text-xs link-red mt-2" onClick={() => { setForgotOpen(false); setForgotSent(false); }}>Back to login</button>
                  </div>
                ) : (
                  <form onSubmit={handleForgotPassword} className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      <Button type="submit" size="sm" className="flex-1 bg-red-600 hover:bg-red-700 text-white" disabled={forgotLoading}>
                        {forgotLoading ? <Loader2 className="w-3 h-3 animate-spin mr-1" /> : <Mail className="w-3 h-3 mr-1" />}
                        Send Reset Link
                      </Button>
                      <Button type="button" size="sm" variant="ghost" onClick={() => setForgotOpen(false)}>Cancel</Button>
                    </div>
                  </form>
                )}
              </div>
