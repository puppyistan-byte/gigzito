...
export interface IStorage {
  ...
  getGroupGoals(groupId: number): Promise<Record<string, unknown>>;
  saveGroupGoals(groupId: number, data: Record<string, unknown>): Promise<void>;
  ...
  // Group Email Invites
  createGroupEmailInvite(groupId: number, email: string, invitedBy: number, token: string, groupName: string, inviterName: string | null): Promise<GroupEmailInvite>;
  getGroupEmailInviteByToken(token: string): Promise<GroupEmailInvite | null>;
  claimGroupEmailInvite(token: string, userId: number): Promise<GroupEmailInvite | null>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByResetToken(token: string): Promise<User | undefined> {
    const [u] = await db.select().from(users).where(eq(users.passwordResetToken, token));
    return u;
  }

  async clearPasswordResetToken(userId: number): Promise<void> {
    await db.update(users).set({ passwordResetToken: null, passwordResetExpiresAt: null }).where(eq(users.id, userId));
  }

  async updateUserEmail(id: number, email: string): Promise<User | undefined> {
    const [u] = await db.update(users).set({ email }).where(eq(users.id, id)).returning();
    return u;
  }
  ...
  async getActiveGzFlashAds(): Promise<GzFlashAdWithOwner[]> {
    const now = new Date();
    const rows = await db
      .select({
        ad: gzFlashAds,
  ...
  async deleteGzFlashAd(id: number, userId: number): Promise<void> {
  ...
  async recalculateGzFlashScores(): Promise<void> {
    const now = new Date();
    const active = await db.select().from(gzFlashAds).where(and(eq(gzFlashAds.status, "active")));
    for (const ad of active) {
      if (ad.expiresAt <= now) {
        await db.update(gzFlashAds).set({ status: "expired", potencyScore: 0 }).where(eq(gzFlashAds.id, ad.id));
      } else {
        const score = this.computePotency(ad.retailPriceCents, ad.discountPercent, ad.quantity, ad.claimedCount, ad.durationMinutes, ad.expiresAt);
  ...
  async getGZMusicComments(trackId: number) {
    const rows = await db
      .select({
        id: gzMusicComments.id,
        trackId: gzMusicComments.trackId,
        userId: gzMusicComments.userId,
        content: gzMusicComments.content,
        createdAt: gzMusicComments.createdAt,
        displayName: providerProfiles.displayName,
        avatarUrl: providerProfiles.avatarUrl,
        username: providerProfiles.username,
      })
      .from(gzMusicComments)
      .leftJoin(providerProfiles, eq(providerProfiles.userId, gzMusicComments.userId))
      .where(eq(gzMusicComments.trackId, trackId))
      .orderBy(desc(gzMusicComments.createdAt));
    return rows;
  }

  async createGZMusicComment(trackId: number, userId: number, content: string) {
    const [comment] = await db.insert(gzMusicComments).values({ trackId, userId, content }).returning();
    return comment;
  }

  async deleteGZMusicComment(id: number, userId: number, isAdmin: boolean) {
    if (isAdmin) {
      await db.delete(gzMusicComments).where(eq(gzMusicComments.id, id));
    } else {
  ...
  async getGroupGoals(groupId: number): Promise<Record<string, unknown>> {
    const [row] = await db.select().from(groupGoals).where(eq(groupGoals.groupId, groupId));
    return (row?.data as Record<string, unknown>) ?? {};
  }

  async saveGroupGoals(groupId: number, data: Record<string, unknown>): Promise<void> {
    await db.insert(groupGoals).values({ groupId, data, updatedAt: new Date() })
      .onConflictDoUpdate({ target: groupGoals.groupId, set: { data, updatedAt: new Date() } });
  }
  ...
  async getEndeavorComments(endeavorId: number) {
    const rows = await db
      .select({ id: groupEndeavorComments.id, endeavorId: groupEndeavorComments.endeavorId, userId: groupEndeavorComments.userId, content: groupEndeavorComments.content, createdAt: groupEndeavorComments.createdAt, displayName: providerProfiles.displayName, avatarUrl: providerProfiles.avatarUrl, username: providerProfiles.username })
      .from(groupEndeavorComments)
      .leftJoin(providerProfiles, eq(providerProfiles.userId, groupEndeavorComments.userId))
      .where(eq(groupEndeavorComments.endeavorId, endeavorId))
      .orderBy(groupEndeavorComments.createdAt);
    return rows.map(r => ({ ...r, createdAt: r.createdAt instanceof Date ? r.createdAt.toISOString() : String(r.createdAt) }));
  }

  async addEndeavorComment(endeavorId: number, userId: number, content: string) {
    const [row] = await db.insert(groupEndeavorComments).values({ endeavorId, userId, content }).returning();
    return { ...row, createdAt: row.createdAt instanceof Date ? row.createdAt.toISOString() : String(row.createdAt) };
  }
  ...
  async getGroupWallets(groupId: number) {
    return db.select().from(groupWallets).where(eq(groupWallets.groupId, groupId)).orderBy(groupWallets.createdAt);
  }

  async createGroupWallet(groupId: number, userId: number, data: { label: string; network: string; address: string; link?: string }) {
    const [row] = await db.insert(groupWallets).values({ groupId, createdBy: userId, label: data.label, network: data.network, address: data.address, link: data.link ?? null, createdAt: new Date() }).returning();
    return row;
  }
  ...
  async createNotification(userId: number, type: string, title: string, message: string, link?: string) {
    const [row] = await db.insert(notifications).values({
      userId, type, title, message, link: link ?? null, isRead: false, createdAt: new Date(),
    }).returning();
    return row;
  }
  ...
  async getUnreadNotificationCount(userId: number) {
    const [{ count }] = await db.select({ count: sql<number>`count(*)::int` }).from(notifications).where(and(eq(notifications.userId, userId), eq(notifications.isRead, false)));
    return count ?? 0;
  }
  ...
  // Group Email Invites
  async createGroupEmailInvite(groupId: number, email: string, invitedBy: number, token: string, groupName: string, inviterName: string | null) {
    const [row] = await db.insert(groupEmailInvites).values({
      groupId, email, token, invitedBy, groupName, inviterName: inviterName ?? null,
      createdAt: new Date(),
  ...
  async getGroupEmailInviteByToken(token: string) {
    const [row] = await db.select().from(groupEmailInvites).where(eq(groupEmailInvites.token, token));
    return row ?? null;
  }

  async claimGroupEmailInvite(token: string, userId: number) {
    const invite = await this.getGroupEmailInviteByToken(token);
    if (!invite || invite.claimedBy || invite.expiresAt < new Date()) return null;
    const [updated] = await db.update(groupEmailInvites)
      .set({ claimedBy: userId, claimedAt: new Date() })
      .where(eq(groupEmailInvites.token, token))
      .returning();
    return updated ?? null;
  }
  ...
  async listGzBands(userId?: number): Promise<GzBandWithMeta[]> {
    const rows = await db.select({
      band: gzBands,
      ...
      .groupBy(gzBands.id)
      .orderBy(desc(gzBands.createdAt));

    if (!userId) return rows.map(r => ({ ...r.band, memberCount: r.memberCount }));

    const myMemberships = await db.select().from(gzBandMembers).where(eq(gzBandMembers.userId, userId));
    const memberMap = new Map(myMemberships.map(m => [m.bandId, m.role]));
    return rows.map(r => ({
      ...r.band,
      memberCount: r.memberCount,
      isMember: memberMap.has(r.band.id),
      memberRole: memberMap.get(r.band.id),
    }));
  }
  ...
  async createGzBand(data: InsertGzBand, userId: number): Promise<GzBand> {
    ...
    const [band] = await db.insert(gzBands).values({ ...data, slug, createdBy: userId }).returning();
    await db.insert(gzBandMembers).values({ bandId: band.id, userId, role: "admin" });
    return band;
  }
  ...
  async getGzBandMember(bandId: number, userId: number) {
    const [m] = await db.select().from(gzBandMembers).where(and(eq(gzBandMembers.bandId, bandId), eq(gzBandMembers.userId, userId)));
    return m ?? null;
  }

  async getGzBandMembers(bandId: number): Promise<GzBandMemberWithProfile[]> {
    const rows = await db.select({
      member: gzBandMembers,
      displayName: providerProfiles.displayName,
      avatarUrl: providerProfiles.avatarUrl,
      username: providerProfiles.username,
      email: users.email,
    }).from(gzBandMembers)
      .leftJoin(providerProfiles, eq(providerProfiles.userId, gzBandMembers.userId))
      .leftJoin(users, eq(users.id, gzBandMembers.userId))
      .where(eq(gzBandMembers.bandId, bandId))
      .orderBy(gzBandMembers.joinedAt);
    return rows.map(r => ({ ...r.member, displayName: r.displayName ?? null, avatarUrl: r.avatarUrl ?? null, username: r.username ?? null, email: r.email! }));
  }

  async joinGzBand(bandId: number, userId: number, instrument?: string): Promise<GzBandMember> {
    const existing = await this.getGzBandMember(bandId, userId);
    if (existing) return existing;
    const [m] = await db.insert(gzBandMembers).values({ bandId, userId, role: "member", instrument: instrument ?? null }).returning();
    return m;
  }

  async leaveGzBand(bandId: number, userId: number): Promise<void> {
  ...
  async getGzBandWallComments(postId: number): Promise<GzBandWallCommentWithAuthor[]> {
    const rows = await db.select({
      comment: gzBandWallComments,
      displayName: providerProfiles.displayName,
      avatarUrl: providerProfiles.avatarUrl,
      username: providerProfiles.username,
    }).from(gzBandWallComments)
      .leftJoin(providerProfiles, eq(providerProfiles.userId, gzBandWallComments.userId))
      .where(eq(gzBandWallComments.postId, postId))
      .orderBy(gzBandWallComments.createdAt);
    return rows.map(r => ({ ...r.comment, displayName: r.displayName ?? null, avatarUrl: r.avatarUrl ?? null, username: r.username ?? null }));
  }

  async createGzBandWallComment(postId: number, userId: number, content: string): Promise<GzBandWallComment> {
    const [comment] = await db.insert(gzBandWallComments).values({ postId, userId, content }).returning();
    return comment;
  }

  async getGzBandEvents(bandId: number): Promise<GzBandEvent[]> {
    return db.select().from(gzBandEvents).where(eq(gzBandEvents.bandId, bandId)).orderBy(gzBandEvents.startAt);
  }

  async createGzBandEvent(bandId: number, data: InsertGzBandEvent, userId: number): Promise<GzBandEvent> {
    const [event] = await db.insert(gzBandEvents).values({ ...data, bandId, createdBy: userId }).returning();
    return event;
  }

  async deleteGzBandEvent(id: number): Promise<void> {
    await db.delete(gzBandEvents).where(eq(gzBandEvents.id, id));
  }

  async getGzBandPhotos(bandId: number): Promise<GzBandPhoto[]> {
    return db.select().from(gzBandPhotos).where(eq(gzBandPhotos.bandId, bandId)).orderBy(desc(gzBandPhotos.createdAt));
  }

  async addGzBandPhoto(bandId: number, url: string, caption: string | null, userId: number): Promise<GzBandPhoto> {
    const [photo] = await db.insert(gzBandPhotos).values({ bandId, url, caption, uploadedBy: userId }).returning();
    return photo;
  }

  async deleteGzBandPhoto(id: number): Promise<void> {
    await db.delete(gzBandPhotos).where(eq(gzBandPhotos.id, id));
  }

  async getGzBandTvShows(bandId: number): Promise<GzBandTvShow[]> {
    return db.select().from(gzBandTvShows).where(eq(gzBandTvShows.bandId, bandId)).orderBy(desc(gzBandTvShows.createdAt));
  }

  async createGzBandTvShow(bandId: number, data: InsertGzBandTvShow): Promise<GzBandTvShow> {
    const [show] = await db.insert(gzBandTvShows).values({ ...data, bandId }).returning();
    return show;
  }

  async deleteGzBandTvShow(id: number): Promise<void> {
    await db.delete(gzBandTvShows).where(eq(gzBandTvShows.id, id));
  }

  async getGzBandTracks(bandId: number) {
  ...
  async getGzBandRoster(bandId: number): Promise<GzBandRosterMember[]> {
